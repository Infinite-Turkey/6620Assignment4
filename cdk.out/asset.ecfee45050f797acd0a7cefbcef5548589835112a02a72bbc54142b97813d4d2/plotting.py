...
def query_size_history():
    ...
    print(f"Querying for size history between {sixty_seconds_ago} and {now} for bucket: {bucket_name}")
    
    response = table.query(
        KeyConditionExpression=boto3.dynamodb.conditions.Key('BucketName').eq(bucket_name) & 
                               boto3.dynamodb.conditions.Key('Timestamp').between(sixty_seconds_ago, now)
    )
...
def plot_size_history(size_data, max_size):
    ...
    plt.plot(timestamps, sizes, label='Bucket Size (last 60s)', marker='o')
    ...
    plt.title('Bucket Size Changes (Last 60 Seconds)')
    plt.xlabel('Time')
